...
#   @output.capture()  # debug
    ...
    def resize_sq(self, bcid):
        ...
        # Newer matplotlib versions use different internal structure
        try:
            # Try newer matplotlib API (3.7+)
            for rect in bcid.ax.patches:
                h = rect.get_height()
                rect.set_height(3*h)
                
            for line in bcid.ax.lines:
                ydata = line.get_ydata()
                if len(ydata) == 2:
                    # Scale the line positions accordingly
                    ymid = (ydata[0] + ydata[1]) / 2
                    yrange = abs(ydata[1] - ydata[0]) * 3
                    line.set_ydata([ymid - yrange/2, ymid + yrange/2])
            ...
            try:
                ...
                # If both methods fail, skip resizing
                pass
