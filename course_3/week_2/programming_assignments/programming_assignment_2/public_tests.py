...
def test_tower(target):
    ...
    for layer in target.layers:
        ...
        # Handle both old and new Keras API for getting output shape
        if hasattr(layer.output, 'shape'):
            output_shape = layer.output.shape.as_list() if hasattr(layer.output.shape, 'as_list') else list(layer.output.shape)
        else:
            output_shape = [None, layer.units]
